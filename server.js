...
// ENDPOINT TEMPORANEO PER TEST EMAIL - DA RIMUOVERE DOPO IL TEST
app.get('/api/test-all-emails', async (req, res) => {
  const testEmail = req.query.email;
  if (!testEmail) return res.status(400).json({ error: 'Parametro email richiesto' });
  ...
  try {
    await sendStatusEmail(testEmail, { ...testData, stato: 'sospesa' });
    results.push({ tipo: 'Iscrizione Sospesa', ok: true });
  } catch (e) { results.push({ tipo: 'Iscrizione Sospesa', ok: false, error: e.message }); }
  
  // 2. Iscrizione in Verifica
  try {
    await sendStatusEmail(testEmail, { ...testData, stato: 'verifica' });
    results.push({ tipo: 'Iscrizione in Verifica', ok: true });
  } catch (e) { results.push({ tipo: 'Iscrizione in Verifica', ok: false, error: e.message }); }
  
  // 3. Iscrizione Confermata
  try {
    await sendStatusEmail(testEmail, { ...testData, stato: 'confermata' });
    results.push({ tipo: 'Iscrizione Confermata', ok: true });
  } catch (e) { results.push({ tipo: 'Iscrizione Confermata', ok: false, error: e.message }); }
  
  // 4. Sollecito Iscrizione
  try {
    await sendStatusEmail(testEmail, { ...testData, stato: 'sollecito' });
    results.push({ tipo: 'Sollecito Iscrizione', ok: true });
  } catch (e) { results.push({ tipo: 'Sollecito Iscrizione', ok: false, error: e.message }); }
  
  // 5. Ricevuta Rigettata
  try {
    await sendStatusEmail(testEmail, { ...testData, stato: 'rigettata', motivo: 'Importo non corrispondente alla quota di iscrizione' });
    results.push({ tipo: 'Ricevuta Rigettata', ok: true });
  } catch (e) { results.push({ tipo: 'Ricevuta Rigettata', ok: false, error: e.message }); }
  
  // 6. Prove Sospesa
  try {
    await sendProveEmail(testEmail, { nome: testData.nome, cognome: testData.cognome, codice: 'PRV-TEST', stato: 'sospesa', atleti: [{ nome: 'Test', cognome: 'Atleta' }], slots: [{ giorno: '2026-11-12', ora: '14:00-14:30' }], totale: 5 });
    results.push({ tipo: 'Prove Sospesa', ok: true });
  } catch (e) { results.push({ tipo: 'Prove Sospesa', ok: false, error: e.message }); }
  
  // 7. Prove Confermata
  try {
    await sendProveEmail(testEmail, { nome: testData.nome, cognome: testData.cognome, codice: 'PRV-TEST', stato: 'confermata', atleti: [{ nome: 'Test', cognome: 'Atleta' }], slots: [{ giorno: '2026-11-12', ora: '14:00-14:30' }], totale: 5 });
    results.push({ tipo: 'Prove Confermata', ok: true });
  } catch (e) { results.push({ tipo: 'Prove Confermata', ok: false, error: e.message }); }
  
  // 8. Merch Sospeso
  try {
    await sendMerchEmail(testEmail, { nome: testData.nome, cognome: testData.cognome, codice: 'MERCH-TEST', stato: 'sospesa', items: [{ articolo: 'Maglia', taglia: 'M', quantita: 1, prezzo: 15 }], totale: 15 });
    results.push({ tipo: 'Merch Sospeso', ok: true });
  } catch (e) { results.push({ tipo: 'Merch Sospeso', ok: false, error: e.message }); }
  
  // 9. Merch Confermato
  try {
    await sendMerchEmail(testEmail, { nome: testData.nome, cognome: testData.cognome, codice: 'MERCH-TEST', stato: 'confermata', items: [{ articolo: 'Maglia', taglia: 'M', quantita: 1, prezzo: 15 }], totale: 15 });
    results.push({ tipo: 'Merch Confermato', ok: true });
  } catch (e) { results.push({ tipo: 'Merch Confermato', ok: false, error: e.message }); }
  
  res.json({ email: testEmail, results });
});
